...
## 관심분야 설정
카테고리 + 유저id = (카테고리 + id) 복합키(+)

관심사 페이지에 categoryId 이용 모든 카테고리 띄우기(+)

userid 활용 저장시 userid의 기록 모두 지우고 선택한 카테고리만 복합키 활용 저장(+)

관심사 페이지 들어가면 선택한 관심사는 체크 표시 해주기(+)

빠른추천 데이터 넘기기(+)

성별, 생년월일은 유저 정보에서 넘기기(-)
...
네비 사이드 바 오류 수정 태그를 바꾸거나 다른 방법 사용(+)
...
회원 탈퇴 시 모든 기록 지우기 (모든 댓글 - 게시글, 기록 등을 지워야 탈퇴 가능)(+)
 
회원 탈퇴 시 댓글 관계 제거 작성자 null로 등록(+)
...
회원정보 수정 후 뒤로가기 누르면 다시 비밀번호 입력창 등장(-)
...
## 달성률
달성률 테이블(+)
userid, 카테고리아이디, 시작날짜, 종료날짜, 설정 목표
키 값 그냥 자동생성해도 될 듯?, 해당 기간 기록에서 카테고리별 포인트만 모아와서 퍼센트로 보여주면 됨

시작일 종료일 기준 날짜 잡기(+)

달성 목표 저장(+)

해당 기간 해당 카테고리에 맞는 포인트로 달성률 계산(+)
...
추천 받을 때 로딩(-)
<!-- 주석처리한 것들은 우선순위가 낮음 나중에 -->

<!-- 기록페이지 설명 -->

<!-- 사이드바 올리고 페이지마다 필요한 것만 사이드에(-) -->
